...
($scope,$location,$sce,notebooksFactory,mainFactory) ->
	...
	$scope.notebooks =  mainFactory.notebooks #keep notebooks up to date by always checking factory when referecing it
	...
	$scope.createNotebook = (name)->
		notebooksFactory.createNotebook(name,mainFactory.user.name)
		$scope.notebooks = mainFactory.notebooks
		$scope.init()
]